...
const PokemonList = () => {
  ...
  if (isLoading) {
    return <div>Carregando os pokemonss........</div>
  }

  if (error) {
    return <h1>Sem pokemons por enquanto</h1>
  }
  ...
  return (
    <main className="p-4">
      <div className="flex justify-between p-4">
        <span className="cursor-pointer" onClick={() => handlePage(-50)}>
          anterior
        </span>
        <span className="cursor-pointer" onClick={() => handlePage(+50)}>
          proximo
      ...
      {data.results.map((pokemon: Pokemon, index: number) => {
        ...
        return (
          <div className="flex px-4 py-2" key={pokemon.name}>
            ...
            <Link className="px-2 uppercase" href={`/pokemon/${id}`}>
              {pokemon.name}
            </Link>
          </div>
        )
